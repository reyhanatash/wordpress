...
function hello_elementor_child_enqueue_scripts() {
	...
	wp_enqueue_script( 'java', get_stylesheet_directory_uri(). '/assets/js/java.js' , array(), '1.5.27', true );
}
...
if (!is_admin() && !is_user_logged_in()) {
    ...
    // ⬛ حذف اسکریپت wp-embed فقط در فرانت‌اند
    add_action('wp_enqueue_scripts', function() {
        wp_deregister_script('wp-embed');
    }, 100);

    // ⬛ فشرده‌سازی ساده HTML خروجی (برای کاربران غیر لاگین فقط در فرانت‌اند)
...
// ⬛ حذف لینک‌های غیرضروری از head (در همه حالت‌ها مشکلی ایجاد نمی‌کنند)
remove_action('wp_head', 'rsd_link');
remove_action('wp_head', 'wlwmanifest_link');

// ⬛ اضافه کردن defer به اسکریپت‌ها (به جز jQuery)
...
// ⬛ غیرفعال‌سازی استایل و اسکریپت Gravity Forms (در صورت استفاده)
add_filter('gform_disable_css', '__return_true');
add_filter('gform_disable_js', '__return_true');

// ⬛ پیش‌لود فونت‌ها و منابع
add_action('wp_head', function() {
    ...
    <!-- ✅ پیش‌لود فونت‌های Google Fonts (در صورت استفاده) -->
